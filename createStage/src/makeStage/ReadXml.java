		...
		if (document.hasChildNodes()) { // 子ノードがある場合
			...
			while (blockNode != null) {
				...
				while (beforeBlockIdNode != null) {
					if (!beforeBlockIdNode.getNodeName()
							.equals("BeforeBlockId")) {
				...
				if (socketsNode != null) {
					...
					while (blockConnectorNode != null) {
						if (!blockConnectorNode.getNodeName().equals(
								"BlockConnector")) {
							blockConnectorNode = blockConnectorNode
									.getNextSibling();
							continue;
						}
						...
						if (name.equals("newDrawer")) {
							...
							while (blockConnectorNode != null) {
								...
								attr = attrs.getNamedItem("con-block-id");
								if (attr != null) {
									conId2 = new Integer(attr.getNodeValue())
											.intValue();
								}
								break;
							}
				...
				block[bi] = new Block(name, id, label, beforeId, afterId,
						conId, conId2);
				bi++;
				blockNode = blockNode.getNextSibling();
			}
			...
			for (bi = 0; bi < bnum; bi++) {
				if (block[bi].name.equals("newDrawer")) {
					...
					drawer[di] = new Drawer(block[bi].label, block[bi], order,
							color);
					di++;
				}
			}
			...
			// 全drawerの中身
			for (di = 0; di < dnum; di++) {
				System.out.println("name = " + drawer[di].name + "  order = "
						+ drawer[di].order + "  color = " + drawer[di].color);
			}
